...
const Navigation: React.FC<{
  ...
  return (
    <nav className="text-black fixed bottom-0 left-0 right-0 india-gradient-smooth border-t border-border z-50 shadow-lg">
...
const Header: React.FC<{
  ...
  return (
    <header className="sticky text-black top-0 z-50 border-b border-border india-gradient-smooth">
...
const NotificationsPanel: React.FC<{
  ...
  const panel = (
    <div
      className="fixed inset-0 z-[9999] bg-black/50 flex items-start justify-end p-4"
      onClick={onClose}
      aria-modal="true"
      role="dialog"
    >
      <div
        className="w-full sm:w-96 max-h-[80vh] bg-white dark:bg-black border border-border rounded-lg shadow-xl overflow-hidden"
...
const AppContent: React.FC = () => {
  ...
  const handleMarkAllRead = async () => {
    ...
    try {
      ...
      await Promise.all(
        unread.map((n) => mockService.markNotificationRead(n.id))
      );
