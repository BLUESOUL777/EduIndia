  ...
  const assignments = [
    {
      ...
      aiScore: 92,
      feedback:
        "Excellent work! Your research shows a deep understanding of desert ecosystems. The report is thorough and well-structured.",
  ...
  const simulateUploadForFile = (file: File | null) => {
    ...
    uploadIntervalRef.current = window.setInterval(() => {
      setUploadProgress((prev) => {
        ...
        const sizeFactor = file
          ? Math.min(4, Math.max(1, file.size / (1024 * 1024)))
          : 1; // 1..4
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
        ...
        >
          ...
          <p className="text-muted-foreground mb-4">
            {t(
              "assignments.dropFile",
              "Drop your assignment file here, or choose an option below"
            )}
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            >
              ...
              {isUploading
                ...
                : t("assignments.browseFiles", "Browse Files")}
            </button>
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-x-4 items-center text-sm text-muted-foreground mb-3">
                  ...
                  {assignment.aiScore && (
                    <div className="flex items-center mb-3">
                      <span className="text-sm text-muted-foreground mr-3">
                        {t("assignments.aiScore", "AI Score")}:
                      </span>
                    ...
                    selectedAssignment === assignment.id.toString() && (
                      <div className="mt-4 p-4 bg-chart-2/10 rounded-lg border-l-4 border-chart-2">
                        <p className="text-sm text-card-foreground">
                          <strong>{t("assignments.feedback")}:</strong>{" "}
                          {assignment.feedback}
                        </p>
                        <p className="text-xs text-muted-foreground mt-2">
                          {t("assignments.reviewedBy", "Reviewed by")}:{" "}
      ...
      <div className="bg-chart-2/5 rounded-xl border border-chart-2/20 p-6 shadow-sm">
        <h3 className="font-semibold text-card-foreground mb-3">
          💡 {t("assignments.submissionTips", "Tips for Better Submissions")}
        </h3>
        <ul className="text-sm text-muted-foreground space-y-2">
          <li>
            ...
            {t(
              "assignments.tip1",
              "Take clear, well-lit photos of handwritten work"
            )}
          </li>
          <li>
            •{" "}
            {t(
              "assignments.tip2",
              "Ensure all text is readable and properly oriented"
            )}
          </li>
          <li>
            • {t("assignments.tip3", "Include your name and assignment title")}
          </li>
          <li>
            •{" "}
            {t(
              "assignments.tip4",
              "Submit before the deadline for best results"
            )}
          </li>
          <li>
            • {t("assignments.tip5", "Use PDF format for typed assignments")}
          </li>
        </ul>
      </div>
    </div>
